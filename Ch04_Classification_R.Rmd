...
```{r,echo=TRUE}
#fit a null model
fit_null = glm(SURVIVED_bin~1, family=binomial, data= Train_COVID)
summary(fit_null)
```
```{r,echo=show_code}
pred_null = predict(fit_null, Test_COVID, type="response")
null_pred <- ifelse(pred_null > 0.5,1,0) # Probability check
CM_n= table(Test_COVID$SURVIVED_bin, null_pred)
print(CM_n)
```
Your answer:
...
Your code:
```{r,echo=TRUE}
fit_1 = glm(SURVIVED_bin~SEX + PATIENT_TYPE + INTUBED + PNEUMONIA + AGE + PREGNANT, family=binomial, data= Train_COVID)
summary(fit_1)
#
```
...
```{r,echo=TRUE}
library(ISLR2)
median_A = median(Auto$mpg)
mpg01 = ifelse(Auto$mpg > median_A, 1, 0)
Auto_ = data.frame(mpg01, Auto)
```
...
Your code:
```{r,echo=TRUE}
size<-floor(dim(Auto_)[1]*0.3)
set.seed(1257)
samp_idx<-sample(1:dim(Auto_)[1],size,FALSE)
Train_Auto<- Auto_[samp_idx,]
Test_Auto <- Auto_[-samp_idx,]
# Sanity check
table(Train_Auto$mpg01)/nrow(Train_Auto)
table(Test_Auto$mpg01)/nrow(Test_Auto)
```
...
```{r,echo=TRUE}
library(MASS)
lda.fit <- lda(mpg01 ~ cylinders + displacement + horsepower + weight+ acceleration + year+ origin, data = Train_Auto)
lda.fit
...
Your code:
```{r,echo=TRUE}
qda.fit <- qda(mpg01 ~ cylinders + displacement + horsepower + weight+ acceleration + year+ origin, data = Train_Auto)
qda.fit
```
...
Your code:
```{r,echo=TRUE}
log_fit = glm(mpg01 ~ cylinders + displacement + horsepower + weight+ acceleration + year+ origin, family = 'binomial',  data = Train_Auto)
log_fit
```
...
```{r,echo=TRUE}
library(e1071)
nb.fit <- naiveBayes(mpg01 ~ cylinders + displacement + horsepower + weight+ acceleration + year+ origin, data = Train_Auto)
nb.fit
#
```
